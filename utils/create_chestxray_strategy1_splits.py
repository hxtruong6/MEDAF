#!/usr/bin/env python3
"""Create ChestX-ray14 Strategy 1 splits for multi-label MEDAF training.

This script implements the splitting strategy described in split.md:
1. Use official NIH train_val_list.txt and test_list.txt for patient-disjoint splits
2. Filter train_val data to exclude images with any new labels (only known labels + "No Finding")
3. Split filtered train_val into train/validation with patient-disjoint split
4. Use full test_list.txt for evaluation (both known and unknown samples)

Outputs:
  * chestxray_strategy1_train.csv - Training data with known labels only
  * chestxray_strategy1_test.csv - Test data with both known and unknown samples
...
def filter_known_only_data(df: pd.DataFrame) -> pd.DataFrame:
    """Filter data to include only images with known labels or 'No Finding'."""
    # Include images with at least one known label OR "No Finding"
    # Exclude images with any new labels
    mask = (df["has_known"] | df["is_no_finding"]) & ~df["has_new"]
    return df[mask].copy()
...
def main(argv: Iterable[str]) -> int:
    ...
    # Filter train_val to exclude images with new labels
    print("🔄 Filtering train_val to known labels only...")
    df_train_val_filtered = filter_known_only_data(df_train_val)
    print(
        f"✅ Filtered train_val: {len(df_train_val_filtered):,} samples (removed {len(df_train_val) - len(df_train_val_filtered):,} with new labels)"
    )

    # Create patient-disjoint train/validation split
    print("🔄 Creating patient-disjoint train/validation split...")
    df_train, df_val = create_patient_disjoint_split(
        df_train_val_filtered, args.val_ratio, args.seed
    )
    ...
    # Save splits
    train_path = args.output_dir / "chestxray_strategy1_train.csv"
    test_path = args.output_dir / "chestxray_strategy1_test.csv"

    # For training, combine train and val (as mentioned in split.md, validation is used for threshold calibration)
    ...
    print("\n🎯 Strategy 1 Split Summary:")
    print(
        f"  • Training: {len(df_train_combined):,} samples (known labels + 'No Finding' only)"
    )
    print(f"  • Test: {len(df_test):,} samples (both known and unknown samples)")
    print(f"  • Patient-disjoint splits: ✅")
    print(f"  • No feature leakage: ✅")
