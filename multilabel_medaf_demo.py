...
class MultiLabelMEDAFDemo:
    ...
    def demo_phase1(self):
        ...
        print(f"Phase 1 Model Parameters: {sum(p.numel() for p in model.parameters() if p.requires_grad):,}")
        ...
        for config_key, config_opts in configurations.items():
            ...
            for epoch in range(self.config['num_epochs']):
                ...
                metrics = train_multilabel_v2(
                    self.train_loader, model, criterion, optimizer,
                    args, self.device, framework
                )
                ...
                if epoch % 2 == 0:
                    print(f"Epoch {epoch}: Loss={metrics['total_loss']:.4f}, Acc={metrics['subset_acc']:.2f}%")
    ...
    def analyze_attention_patterns(self):
        ...
        inputs, targets = sample_batch[0][:4].to(self.device), sample_batch[1][:4].to(self.device)
        ...
        with torch.no_grad():
            ...
            global_outputs = global_model(inputs, targets, return_attention_weights=True)
            ...
            if 'per_class_weights' in pc_outputs:
                ...
                for class_idx in range(avg_class_prefs.shape[0]):
                    ...
                    print(f"  Class {class_idx}: Expert {dominant_expert} ({max_pref:.3f}) - {expert_prefs}")
                
                # Measure specialization
                expert_entropy = -(avg_class_prefs * torch.log(avg_class_prefs + 1e-8)).sum(dim=-1)
                avg_entropy = expert_entropy.mean().item()
                
                print(f"\nSpecialization Analysis:")
                print(f"Average gating entropy: {avg_entropy:.3f} (lower = more specialized)")
    ...
    def print_final_summary(self):
        ...
        if 'phase2' in self.results:
            ...
            for config_key, results in self.results['phase2'].items():
                ...
                print(f"   {config_name:20}: Acc={best_acc:6.2f}%, Loss={final_loss:.4f}")
            
            # Find best configuration
            best_config = max(
                self.results['phase2'].items(),
            ...
            if 'global_gating' in self.results['phase2'] and 'per_class_gating' in self.results['phase2']:
        ...
        print(f"\n🔧 Configuration Used:")
        print(f"   Dataset: {self.config['num_samples']} samples, {self.config['num_classes']} classes")
        print(f"   Training: {self.config['num_epochs']} epochs, batch size {self.config['batch_size']}")
        ...
        print(f"   • Attention diversity encourages experts to focus on different regions")
