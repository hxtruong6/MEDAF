...
def quick_analysis(csv_path):
    ...
    print(
        f"   Age range: {df['Patient Age Numeric'].min()}-{df['Patient Age Numeric'].max()} years"
    )
    ...
    counts = [
        label_counts[label] for label in pathology_labels if label_counts[label] > 0
    ]
    ...
    print(
        f"   Most common: {max(label_counts, key=label_counts.get)} ({max_count:,} cases)"
    )
    print(
        f"   Least common: {min(label_counts, key=label_counts.get)} ({min_count:,} cases)"
    )
