...
class ChestXrayAnalyzer:
    ...
    def load_data(self):
        ...
        self.df['Patient Age Numeric'] = self.df['Patient Age'].str.replace('Y', '').astype(int)
        
        # Parse image dimensions
        self.df['Image Width'] = self.df['OriginalImage[Width,Height]'].str.split(',').str[0].astype(int)
        self.df['Image Height'] = self.df['OriginalImage[Width,Height]'].str.split(',').str[1].astype(int)
    ...
    def basic_statistics(self):
        ...
        print(f"   • Date range: {self.df['Follow-up #'].min()} - {self.df['Follow-up #'].max()}")
        
        print(f"\n👥 Patient Demographics:")
        print(f"   • Male patients: {len(self.df[self.df['Patient Gender'] == 'M']):,} ({len(self.df[self.df['Patient Gender'] == 'M'])/len(self.df)*100:.1f}%)")
        print(f"   • Female patients: {len(self.df[self.df['Patient Gender'] == 'F']):,} ({len(self.df[self.df['Patient Gender'] == 'F'])/len(self.df)*100:.1f}%)")
        print(f"   • Average age: {self.df['Patient Age Numeric'].mean():.1f} years")
        print(f"   • Age range: {self.df['Patient Age Numeric'].min()} - {self.df['Patient Age Numeric'].max()} years")
        ...
        print(f"   • Average image size: {self.df['Image Width'].mean():.0f} x {self.df['Image Height'].mean():.0f}")
        print(f"   • Image size range: {self.df['Image Width'].min()}x{self.df['Image Height'].min()} - {self.df['Image Width'].max()}x{self.df['Image Height'].max()}")
    ...
    def label_analysis(self):
        ...
        sorted_labels = sorted(label_counts.items(), key=lambda x: x[1]['count'], reverse=True)
    ...
    def multi_label_analysis(self):
        ...
        print(f"\n📈 Multi-label Statistics:")
        print(f"   • Single label images: {single_label:,} ({single_label/len(self.df)*100:.1f}%)")
        print(f"   • Multi-label images: {multi_label:,} ({multi_label/len(self.df)*100:.1f}%)")
        print(f"   • No finding images: {no_finding:,} ({no_finding/len(self.df)*100:.1f}%)")
    ...
    def label_cooccurrence_analysis(self):
        ...
        for i, label1 in enumerate(label_columns):
            for j, label2 in enumerate(label_columns):
                if i != j:
                    ...
                    both_labels = len(self.df[(self.df[label1] == 1) & (self.df[label2] == 1)])
        ...
        for i in range(len(label_columns)):
            for j in range(i+1, len(label_columns)):
                ...
                if count > 0:
                    cooccurrence_pairs.append((label_columns[i], label_columns[j], count))
        ...
        for gender in ['M', 'F']:
            if gender in gender_analysis.index:
                ...
                for label, count in gender_counts.head(10).items():
                    if count > 0:
                        percentage = (count / len(self.df[self.df['Patient Gender'] == gender])) * 100
                        print(f"     {label:<20} {count:<8,} ({percentage:.1f}%)")
        
        # Age group analysis
        print(f"\n📅 Pathology Distribution by Age Groups:")
        age_groups = pd.cut(self.df['Patient Age Numeric'], 
                           bins=[0, 30, 50, 70, 100], 
    ...
    def create_visualizations(self, save_plots=True):
        ...
        plt.subplot(4, 2, 1)
        label_counts = [self.label_stats[label]['count'] for label in self.pathology_labels + ['No Finding']]
        label_names = self.pathology_labels + ['No Finding']
        
        bars = plt.bar(range(len(label_names)), label_counts, color='skyblue', alpha=0.7)
        ...
        plt.subplot(4, 2, 2)
        num_labels_dist = self.df['Num_Labels'].value_counts().sort_index()
        plt.bar(num_labels_dist.index, num_labels_dist.values, color='lightcoral', alpha=0.7)
        ...
        # 4. Age distribution
        plt.subplot(4, 2, 4)
        plt.hist(self.df['Patient Age Numeric'], bins=20, color='lightgreen', alpha=0.7, edgecolor='black')
        ...
        if top_combinations:
            combo_names = [combo[:20] + '...' if len(combo) > 20 else combo for combo, _ in top_combinations]
        ...
        if not age_pathology.empty:
            ...
            for age_group in age_pathology.index:
                if pd.notna(age_group) and age_group in age_group_sizes:
                    age_pathology.loc[age_group] = age_pathology.loc[age_group] / age_group_sizes[age_group] * 100
            
            sns.heatmap(age_pathology.T, annot=True, fmt='.1f', cmap='YlOrRd', cbar_kws={'label': 'Percentage'})
            plt.title('Pathology Prevalence by Age Group (%)', fontsize=14, fontweight='bold')
    ...
    def generate_summary_report(self):
        ...
        pathology_counts = {label: self.df[label].sum() for label in self.pathology_labels}
        most_common = max(pathology_counts, key=pathology_counts.get)
        least_common = min(pathology_counts, key=pathology_counts.get)
        
        print(f"🎯 Key Findings:")
        print(f"   • Dataset contains {total_images:,} chest X-ray images from {total_patients:,} patients")
        print(f"   • {multi_label_images:,} images ({multi_label_images/total_images*100:.1f}%) have multiple pathologies")
        print(f"   • {no_finding_images:,} images ({no_finding_images/total_images*100:.1f}%) show no abnormalities")
        print(f"   • Most common pathology: {most_common} ({pathology_counts[most_common]:,} cases)")
        print(f"   • Least common pathology: {least_common} ({pathology_counts[least_common]:,} cases)")
        print(f"   • Average {self.df['Num_Labels'].mean():.2f} labels per image")
        
        print(f"\n📊 Dataset Characteristics:")
        print(f"   • Multi-label classification problem with {len(self.pathology_labels)} pathology classes")
        print(f"   • Significant class imbalance (most common vs least common: {pathology_counts[most_common]/pathology_counts[least_common]:.1f}x)")
        print(f"   • Patient demographics: {len(self.df[self.df['Patient Gender'] == 'M'])/len(self.df)*100:.1f}% male, {len(self.df[self.df['Patient Gender'] == 'F'])/len(self.df)*100:.1f}% female")
        print(f"   • Age range: {self.df['Patient Age Numeric'].min()}-{self.df['Patient Age Numeric'].max()} years (avg: {self.df['Patient Age Numeric'].mean():.1f})")
