...
def enhancedMultiLabelAttnDiv(
    cams_list, targets, gate_weights=None, diversity_type="cosine", eps=1e-6
):
    """
            ...
            if diversity_type == "cosine":
                ...
                for i in range(3):
                    for j in range(i + 1, 3):
                        ...
                        diversity_loss -= (
                            l2_dist  # Negative because we want to maximize distance
                        )
                        total_pairs += 1
...
class ComparativeTrainingFramework:
    ...
    def get_comparison_summary(self):
        ...
        for model_type, results in self.results.items():
            ...
            summary[model_type] = {
                ...
                "avg_diversity_loss": np.mean(
                    [r.get("diversity_loss", 0) for r in results[-10:]]
                ),
            }
    ...
    def print_comparison(self):
        ...
        if "global" in summary and "per_class" in summary:
            ...
            print(
                f"{'Metric':<20} {'Global':<15} {'Per-Class':<15} {'Improvement':<15}"
            )
            print("-" * 65)
            ...
            for metric in metrics:
                ...
                print(
                    f"{metric.replace('final_', ''):<20} {global_val:<15.4f} {pc_val:<15.4f} {improvement:+.2f}%"
                )
            ...
            print(
                f"{'loss_reduction':<20} {global_loss:<15.4f} {pc_loss:<15.4f} {loss_improvement:+.2f}%"
            )

        print("=" * 60)


def train_multilabel_v2(
    train_loader,
    model,
    criterion,
    optimizer,
    ...
    for i, data in enumerate(train_loader):
        ...
        if per_class_weights is not None and args.get("gating_regularization", 0) > 0:
            ...
            gate_entropy = -(
                per_class_weights * torch.log(per_class_weights + 1e-8)
            ).sum(dim=-1)
            gating_reg_loss = (
                -gate_entropy.mean()
            )  # Negative to encourage high entropy (diversity)
    ...
    if comparative_framework is not None:
        comparative_framework.log_metrics(
            gating_type, comparative_framework.current_epoch, final_metrics
        )
...
def evaluate_multilabel_v2(model, test_loader, criterion, args, device=None):
    ...
    if all_gate_weights:
        ...
        expert_entropy = -(
            expert_preferences * torch.log(expert_preferences + 1e-8)
        ).sum(dim=-1)
