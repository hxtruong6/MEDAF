...
def calculate_class_weights(train_loader, num_classes, device, method="inverse_freq"):
    """
...
def optimize_thresholds_per_class(
    model, val_loader, device, num_classes, class_names=None
):
    """
...
def evaluate_with_optimal_thresholds(
    model, test_loader, device, optimal_thresholds, class_names=None
):
    """
...
def train_multilabel_enhanced(
    ...
    for i, data in enumerate(train_loader):
        ...
        acc_values = []
        for logit in logits:
            subset_acc, hamming_acc, _, _, _ = multiLabelAccuracy(
