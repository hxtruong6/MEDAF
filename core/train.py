...
def attnDiv(cams):
    ...
    mean = (
        cams.mean(dim=-1)
        .view(bs, num_part, -1)
        .expand(size=[bs, num_part, cams.shape[-1]])
    )
    ...
    for i in range(cams.shape[1]):
        for j in range(i + 1, cams.shape[1]):
            orthogonal_loss += cos(
                cams[:, i, :].view(bs, 1, -1), cams[:, j, :].view(bs, 1, -1)
            ).mean()
...
def train(train_loader, model, criterion, optimizer, args, device=None):
    ...
    for i, data in enumerate(train_loader):
        ...
        loss_values = [
            criterion["entropy"](logit.float(), target.long()) for logit in logits
        ]
        loss_values.append(attnDiv(branch_cams))
        loss_values.append(
            args["loss_wgts"][0] * sum(loss_values[:3])
            + args["loss_wgts"][1] * loss_values[-2]
            + args["loss_wgts"][2] * loss_values[-1]
        )
