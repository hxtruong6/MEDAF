...
class ResNet(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(
                img_chan, 64, kernel_size=3, padding=1, bias=False, stride=resprestride
            ),
            nn.BatchNorm2d(64),
            nn.LeakyReLU(),
        )
        ...
        self.conv6_x = (
            nn.Identity() if output_dim <= 0 else self.conv_layer(512, output_dim, 1, 0)
        )
    ...
    def conv_layer(self, input_channel, output_channel, kernel_size=3, padding=1):
        ...
        res = nn.Sequential(
            nn.Conv2d(
                input_channel, output_channel, kernel_size, 1, padding, bias=False
            ),
...
class BaselineNet(nn.Module):
    def __init__(self, args):
        ...
        backbone, feature_dim, _ = build_backbone(
        ...
        if y is not None:
            cams = torch.cat(
                [
                    b1_ft_cams.gather(
                        dim=1,
                        index=y[:, None, None, None].repeat(
                            1, 1, b1_ft_cams.shape[-2], b1_ft_cams.shape[-1]
                        ),
                    ),
                    b2_ft_cams.gather(
                        dim=1,
                        index=y[:, None, None, None].repeat(
                            1, 1, b2_ft_cams.shape[-2], b2_ft_cams.shape[-1]
                        ),
                    ),
                    b3_ft_cams.gather(
                        dim=1,
                        index=y[:, None, None, None].repeat(
                            1, 1, b3_ft_cams.shape[-2], b3_ft_cams.shape[-1]
                        ),
                    ),
                ],
                dim=1,
