...
def getLoader(options):
    ...
    if "cifar10" == options["dataset"]:
        ...
        train_loader, test_loader, out_loader = (
            Data.train_loader,
            Data.test_loader,
            Data.out_loader,
        )
        ...
        out_Data = CIFAR100_OSR(
            known=options["unknown"],
            batch_size=options["batch_size"],
            img_size=options["img_size"],
            options=options,
        )
...
def main(options):
    ...
    for i in range(len(splits[options["dataset"]])):
        ...
        stats_log.write(
            "SPLIT[%d|5] => Accuracy: [%.3f], AUROC: [%.3f], AUPR_IN: [%.3f], AUPR_OUT: [%.3f], F1-score: [%.3f]\n"
            % (
                i + 1,
                temp_result[0],
                temp_result[1],
...
def trainLoop(options):
    ...
    if options["optimizer"] == "SGD":
        optimizer = torch.optim.SGD(
            params, lr=options["lr"], momentum=0.9, weight_decay=1e-5
        )
        ...
        optimizer = torch.optim.AdamW(
            params, lr=options["lr"], betas=(0.9, 0.99), weight_decay=1e-5
        )
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            optimizer, milestones=options["milestones"], gamma=options["gamma"]
        )
