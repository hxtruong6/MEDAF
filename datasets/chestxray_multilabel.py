...
try:
    ...
    XRV_AVAILABLE = False
    raise ImportError("TorchXRayVision is required. Install with: pip install torchxrayvision")
...
class ChestXRayMultiLabel(data.Dataset):
    ...
    def __init__(self, split='train', transform=None, dataset_name='nih', 
                 data_root="./datasets/data/chestxray", max_samples=None, train_ratio=0.8):
        
        if not XRV_AVAILABLE:
            raise ImportError("TorchXRayVision is required but not installed. "
    ...
    def _load_xrv_dataset(self, dataset_name):
        ...
        if dataset_name.lower() == 'nih':
            ...
            dataset = xrv.datasets.MIMIC_Dataset(
                ...
                metacsvpath=str(self.data_root / "MIMIC" / "mimic-cxr-2.0.0-chexpert.csv"),
                views=["PA", "AP"]
            )
    ...
    def _process_dataset(self, max_samples):
        ...
        indices = np.random.RandomState(42).permutation(len(self.dataset))[:total_samples]
...
def create_chestxray_dataloaders(dataset_name='nih', data_root="./datasets/data/chestxray",
                                batch_size=32, img_size=224, num_workers=4, max_samples=None,
                                train_ratio=0.8):
    """
...
def check_dataset_availability(dataset_name='nih', data_root="./datasets/data/chestxray"):
    ...
    if dataset_name.lower() == 'nih':
        ...
        if imgpath.exists() and csvpath.exists():
            ...
            return False, "NIH dataset not found. Run: python download_real_chestxray.py"
...
if __name__ == "__main__":
    ...
    try:
        ...
        positive_pathologies = [pathologies[i] for i, val in enumerate(first_labels) if val > 0.5]
