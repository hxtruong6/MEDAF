...
class PascalVOC2007MultiLabel(data.Dataset):
    ...
    def __init__(self, root, split='train', transform=None, download=True):
        ...
        print(f"Average labels per image: {np.mean([label.sum() for label in self.labels]):.2f}")
    ...
    def _create_synthetic_voc(self):
        ...
        for i in range(num_samples):
            ...
            selected_classes = np.random.choice(self.num_classes, num_labels, replace=False)
            label[selected_classes] = 1.0
            
            labels.append(label)
        
        print(f"Generated {num_samples} synthetic PASCAL VOC samples for {self.split} split")
        return image_paths, labels
...
def create_multilabel_dataloaders(dataset_name, data_root, batch_size=32, 
