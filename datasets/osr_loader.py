...
class CIFAR10_Filter(CIFAR10):
    def __Filter__(self, known):
        ...
        self.data, self.targets = np.squeeze(np.take(datas, mask, axis=0)), np.array(
            new_targets
        )
...
class SVHN_OSR(object):
    def __init__(
        ...
        trainset = SVHN_Filter(
            root=dataroot, split="train", download=True, transform=train_transform
        )
        trainset.__Filter__(known=self.known)
        self.train_loader = torch.utils.data.DataLoader(
            trainset,
        ...
        testset = SVHN_Filter(
            root=dataroot, split="test", download=True, transform=transform
        )
        testset.__Filter__(known=self.known)
        self.test_loader = torch.utils.data.DataLoader(
            testset,
...
class Tiny_ImageNet_OSR(object):
    def __init__(
        ...
        trainset = Tiny_ImageNet_Filter(
            os.path.join(dataroot, "tiny-imagenet-200", "train"), train_transform
        )
        trainset.__Filter__(known=self.known)
        self.train_loader = torch.utils.data.DataLoader(
            trainset,
        ...
        testset = Tiny_ImageNet_Filter(
            os.path.join(dataroot, "tiny-imagenet-200", "val"), transform
        )
        testset.__Filter__(known=self.known)
        self.test_loader = torch.utils.data.DataLoader(
            testset,
